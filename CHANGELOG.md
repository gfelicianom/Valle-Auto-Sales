...
- Claude added `.github/workflows/audit-airtable-photos.yml`, a read-only button that publishes the audit to the run summary. It takes an optional `fail_over_mb` and fails the run once that much storage is reclaimable, which is what a monthly `schedule:` would need to turn it into an early warning — Chelsea is leaving the schedule off until the manual runs have been exercised once.
